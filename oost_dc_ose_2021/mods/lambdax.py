...
def lambdax(
    ...
    preprocess_ref: Callable[[xr.Dataset], xr.DataArray] = _b(
        operator.attrgetter, "ssh"
    ),
    preprocess_study: Callable[[xr.Dataset], xr.DataArray] = _b(
        operator.attrgetter, "ssh"
    ),
):
    r"""
    ...
    _log.debug("Computing track segments")
    segments = eval_ds.pipe(partial_track_fn)
    _log.debug(f"{segments=}")
    ds, _ = segments.pipe(partial_score_fn)
    _log.debug("Computing the PSD (welch score)")
    _log.debug(f"PSD ds {ds}")

    ## Robust lambda_x computation when small wavelength reach score > 0.5
    lambda_x = 1 / min_intersect(ds)
    _log.info(f"Effective scale resolved (interpolated at score 0.5) {lambda_x:.2f}")
    ...
    _log.debug(f"Writing PSD ds to {output_psd_path}")
    ds.to_netcdf(output_psd_path)
    _log.debug(f"Lambda score to {output_lambdax_path}")
